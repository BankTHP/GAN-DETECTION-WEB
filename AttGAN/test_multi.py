...
def parse(args=None):
    ...
    parser.add_argument('--custom_attr', type=str)
...
with open(join('AttGAN/output', args_.experiment_name, 'setting.txt'), 'r') as f:
...
if args.custom_img:
    output_path = join('AttGAN/output', args.experiment_name, 'custom_testing_multi_' + str(args.test_atts))
    from data import Custom
    test_dataset = Custom(args.custom_data, args.custom_attr, args.img_size, args.attrs)
else:
    output_path = join('AttGAN/output', args.experiment_name, 'sample_testing_multi_' + str(args.test_atts))
    if args.data == 'CelebA':
